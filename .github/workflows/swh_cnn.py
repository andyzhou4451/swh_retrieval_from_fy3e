...
def train_and_evaluate_model(train_loader, valid_loader, test_loader, model, criterion, optimizer, num_epochs=500):
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            best_model_path = os.path.join(c2net_context.output_path, 'best_model.pth')
    ...
    with open(result_file, 'w') as f:
        f.write(f"Training complete. Best Validation Loss: {best_valid_loss:.4f}\n")
    ...
    print('Training completed. Best Validation Loss:', best_valid_loss)
...
np.savez(os.path.join(c2net_context.output_path, 'final_results.npz'),
